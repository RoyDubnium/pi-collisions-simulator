...
struct MainState {
    ...
    /// Sound to play whenever a collision occurs
    snapshot_sound: Source,
}
...
impl EventHandler for MainState {
    ...
    fn draw(&mut self, ctx: &mut Context) -> GameResult {
        ...
        if let Some((snapshot_time, snapshot)) = self.get_latest_snapshot(current_time) {
            if self.last_snapshot_time.map_or(true, |t| (snapshot_time - t).abs() > std::f64::EPSILON) {
                // Play the sound in detached mode.
                self.snapshot_sound.play_detached(ctx)?;
